...
const fetchClerkUsers = async (): Promise<IUser[]> => {
  try {
    ...
    while (hasMore) {
      ...
      const mappedUsers: IUser[] = response.map((user) => ({
        id: user.id,
      ...
      offset += limit;
      hasMore = response.length === limit; // Si se devolvió el máximo, asumimos que hay más
    }

    // Ordenar por nombre completo
    allUsers.sort((a, b) => a.fullName.localeCompare(b.fullName, "es", { sensitivity: "base" }));
...
export const getClerkUsers = fetchClerkUsers
