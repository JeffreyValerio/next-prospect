  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
      {stats.map((stat, index) => (
        <Card key={index} className={`${stat.borderColor} dark:border-gray-700 border-l-4`}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-600 dark:text-gray-400">
              {stat.title}
            </CardTitle>
            <div className={`p-2 rounded-full ${stat.bgColor} dark:bg-gray-800`}>
              <stat.icon className={`h-4 w-4 ${stat.color} dark:text-gray-300`} />
            </div>
          </CardHeader>
          <CardContent>
      ...
      {selected > 0 && (
        <Card className="border-l-4 border-l-blue-500 dark:border-l-blue-600 bg-blue-50 dark:bg-blue-950">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-2 mt-2">
              <Badge variant="default" className="text-xs bg-blue-600 dark:bg-blue-700">
                Seleccionados
              </Badge>
            </div>
          </CardContent>
